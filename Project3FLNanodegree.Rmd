...
```
------ 
Volatile Acidity seems to be one of the primary contributors to quality, the box plot aids to reveal the linear nature of this input variable to the dependent. An added observation/visualization is that citric acid is negative correlation of Citric Acid and Volatile Acidity.
...
p1 <- ggplot(aes(x = Row.Names, y = Proportion.of.Variance, 
  ...
  labs(x='Principal Components', y='Proportion of Var') + 
...
p2 <- ggplot(aes(x = Row.Names, y = Cumulative.Variance), 
             data = tvarProp) + geom_bar(stat='identity') +
  labs(x='Principal Components', y = 'Cumulative Var') + 
  ggtitle('PCA Cumulative Variance') +
  theme(axis.text.x = element_text(angle = 25, hjust = 1))


p3 <- ggplot(aes(x = Row.Names, y = value, color = PCs, group = PCs), 
             data = tidyPCA) + 
  geom_point() + geom_line(size = 1, alpha = .7, linetype = 2) + 
  labs(x='Features', y='PC Weights') + 
  ggtitle('Principal Components Weight Breakdown by PCs') +
  theme(axis.text.x = element_text(angle = 20, hjust = 1), 
        legend.text = element_text(size = 8)) +
  scale_color_brewer(type = 'seq', palette = 'Spectral',
    guide = guide_legend(title = 'PCs', reverse = F,
    override.aes = list(alpha = 1, size = 3)))
...
```
------ 
The final low hanging insight is that alcohol is linearly correlated with quality and should be a significant contributor if one was to build a predictive model around the given variables and red wine quality. It seems that at least for red wine, higher alcohol content means better quality score! 
